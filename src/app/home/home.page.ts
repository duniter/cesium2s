...
import {
  ...
  LocaleConfig,
  Settings,
} from '@app/settings/settings.model';
...
export class HomePage extends BasePage<Settings> implements OnInit {
  ...
  @ViewChild('loginModalV2') loginModalV2: IonModal;
  ...
  protected loginMethods: LoginMethod[] = [
    ...
    {
      value: 'keyfile-v1',
      label: 'Fichier de clef Duniter v1',
      disabled: true,
    },
  ];
  ...
  async login(event) {
    ...
    if (!this.mobile) {
      ...
      const actionSheet = await this.actionSheetCtrl.create({
        ...
        buttons: this.loginMethods.map((method) => <ActionSheetButton>{
            data: method.value,

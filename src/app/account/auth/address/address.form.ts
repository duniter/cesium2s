...
export class AddressForm extends AppForm<AuthData> implements OnInit {
  constructor(
    ...
    super(
      formBuilder.group({
        address: [null, Validators.compose([Validators.required, SharedValidators.address])],
      })
    );
  ...
  async showWotModal(event: Event) {
    event.preventDefault();
    const searchText = this.form.get('address').value;
